...
func NewServiceDependencies(ctx context.Context, p Params) (*ServiceDependencies, error) {
	...
	// Build a PermissionManager and JujuAuthFactory so the dialer can mint
	// caller-scoped JWT tokens without going through the full JIMM struct.
	// JIMM.New constructs its own stateless Factory for its other uses.
