...
func TestControllerProfileCRUD(t *testing.T) {
	...
	saved, err := client.SaveControllerProfile(t.Context(), &req)
	...
	got, err := client.GetControllerProfile(t.Context(), &apiparams.GetControllerProfileRequest{Name: req.Name})
	c.Assert(err, qt.IsNil)
	...
	updated, err := client.SaveControllerProfile(t.Context(), &apiparams.SaveControllerProfileRequest{ControllerProfile: saved.ControllerProfile})
	c.Assert(err, qt.IsNil)
	c.Assert(updated.Version, qt.Equals, uint(2))
	c.Assert(updated.Description, qt.Equals, "Updated reusable bootstrap settings")

	profiles, err := client.ListControllerProfiles(t.Context(), &apiparams.ListControllerProfilesRequest{})
	c.Assert(err, qt.IsNil)
	...
	err = client.RemoveControllerProfile(t.Context(), &apiparams.RemoveControllerProfileRequest{Name: req.Name})
	c.Assert(err, qt.IsNil)

	_, err = client.GetControllerProfile(t.Context(), &apiparams.GetControllerProfileRequest{Name: req.Name})
...
func TestControllerProfileValidation(t *testing.T) {
	...
	_, err := client.SaveControllerProfile(t.Context(), &builtIn)
	...
	_, err = client.SaveControllerProfile(t.Context(), &malformedPool)
	...
	_, err = client.SaveControllerProfile(t.Context(), &missingVersion)
	...
	_, err = client.SaveControllerProfile(t.Context(), &invalidVersion)
	...
	_, err = client.SaveControllerProfile(t.Context(), &buildVersion)
	c.Assert(err, qt.ErrorMatches, ".*partial/full version string with one, two, or three dot-separated numeric components.*")

	taggedVersion := testControllerProfileRequest("tagged-version")
	taggedVersion.JujuVersion = "3.6-beta1"
	_, err = client.SaveControllerProfile(t.Context(), &taggedVersion)
	c.Assert(err, qt.ErrorMatches, ".*partial/full version string with one, two, or three dot-separated numeric components.*")

	_, err = client.ListControllerProfiles(t.Context(), &apiparams.ListControllerProfilesRequest{JujuVersion: "3.6.x"})
	...
	_, err = client.ListControllerProfiles(t.Context(), &apiparams.ListControllerProfilesRequest{JujuVersion: "3.6.4.1"})
	c.Assert(err, qt.ErrorMatches, ".*juju version filter must be a partial/full version string with one, two, or three dot-separated numeric components.*")

	_, err = client.ListControllerProfiles(t.Context(), &apiparams.ListControllerProfilesRequest{JujuVersion: "3.6-beta1"})
	c.Assert(err, qt.ErrorMatches, ".*juju version filter must be a partial/full version string with one, two, or three dot-separated numeric components.*")
}
...
func TestControllerProfileListFiltering(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		_, err := client.SaveControllerProfile(t.Context(), &req)
		c.Assert(err, qt.IsNil)
	}

	profiles, err := client.ListControllerProfiles(t.Context(), &apiparams.ListControllerProfilesRequest{JujuVersion: "3.6.4"})
	c.Assert(err, qt.IsNil)
...
func TestControllerProfileUnauthorized(t *testing.T) {
	...
	_, err := adminClient.SaveControllerProfile(t.Context(), &req)
	c.Assert(err, qt.IsNil)
	...
	_, err = client.SaveControllerProfile(t.Context(), &unauthorizedReq)
	c.Assert(err, qt.ErrorMatches, ".*unauthorized.*")

	_, err = client.GetControllerProfile(t.Context(), &apiparams.GetControllerProfileRequest{Name: req.Name})
	c.Assert(err, qt.ErrorMatches, ".*unauthorized.*")

	_, err = client.ListControllerProfiles(t.Context(), &apiparams.ListControllerProfilesRequest{})
	c.Assert(err, qt.ErrorMatches, ".*unauthorized.*")
