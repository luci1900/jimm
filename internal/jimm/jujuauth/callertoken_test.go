...
// TestCallerLoginTokenForNonAdminUser verifies that NewScopedLoginToken
...
func TestCallerLoginTokenForNonAdminUser(t *testing.T) {
	c := qt.New(t)
...
// TestCallerLoginTokenForAdminUser verifies that an admin user still
// receives controller superuser access in the caller token, matching
// the old hardcoded behaviour for the admin case.
func TestCallerLoginTokenForAdminUser(t *testing.T) {
	c := qt.New(t)
	env := jimmtest.SetupJimmEnv(c)
	ctx := c.Context()
