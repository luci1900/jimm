...
func (j *JujuManager) Offer(ctx context.Context, user *openfga.User, offer AddApplicationOfferParams) error {
	...
	if err == nil {
		...
		checkAPI, dialErr := j.dialController(ctx, &model.Controller, names.ModelTag{}, user)
		if dialErr != nil {
			return dialErr
		}
	...
	api, err := j.dialController(ctx, &model.Controller, names.ModelTag{}, user)
	if err != nil {
		return err
	}
	defer api.Close()
...
func (j *JujuManager) GetApplicationOfferConsumeDetails(ctx context.Context, user *openfga.User, details *jujuparams.ConsumeOfferDetails, v bakery.Version) error {
	...
	api, err := j.dialController(ctx, &offer.Model.Controller, names.ModelTag{}, user)
	if err != nil {
		return err
	}
	defer api.Close()
