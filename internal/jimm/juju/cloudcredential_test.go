...
func TestUpdateCloudCredential(t *testing.T) {
	...
	tests := []struct {
		...
		createEnv: func(c *qt.C, j *juju.JujuManager) (*dbmodel.Identity, juju.UpdateCloudCredentialArgs, dbmodel.CloudCredential, string) {
			...
			mi, err := j.AddModel(context.Background(), user, &juju.ModelCreateArgs{
				Name:            "test-model",
			...
			arg := juju.UpdateCloudCredentialArgs{
		...
		createEnv: func(c *qt.C, j *juju.JujuManager) (*dbmodel.Identity, juju.UpdateCloudCredentialArgs, dbmodel.CloudCredential, string) {
			...
			_, err = j.AddModel(context.Background(), user, &juju.ModelCreateArgs{
				Name:            "test-model",
				Owner:           names.NewUserTag(u.Name),
				Cloud:           names.NewCloudTag(cloud.Name),
				CloudRegion:     "test-region-1",
		...
		createEnv: func(c *qt.C, j *juju.JujuManager) (*dbmodel.Identity, juju.UpdateCloudCredentialArgs, dbmodel.CloudCredential, string) {
			...
			mi, err := j.AddModel(context.Background(), alice, &juju.ModelCreateArgs{
				Name:            "test-model",
...
func TestRevokeCloudCredential(t *testing.T) {
	...
	tests := []struct {
		...
		createEnv: func(c *qt.C, j *juju.JujuManager, client *openfga.OFGAClient) (*dbmodel.Identity, names.CloudCredentialTag, string) {
			...
			_, err = j.AddModel(context.Background(), alice, &juju.ModelCreateArgs{
				Name:            "test-model",
				Owner:           names.NewUserTag(u.Name),
				Cloud:           names.NewCloudTag(cloud.Name),
				CloudRegion:     "test-region-1",
...
func TestGetCloudCredentialAttributes(t *testing.T) {
	...
	for _, test := range getCloudCredentialAttributesTests {
		...
		jimmWithVault := parameters{CredentialStore: vaultStore}

		testF := func(jp *parameters) func(c *qt.C) {
			return func(c *qt.C) {
				ctx := context.Background()
...
`)
	...
	u := env.User("alice@canonical.com").DBObject(c, j.Database)
	user := openfga.NewUser(&u, j.OpenFGAClient)
	args := juju.UpdateCloudCredentialArgs{
