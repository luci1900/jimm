...
func (j *JujuManager) AbortMigration(ctx context.Context, user *openfga.User, modelUUID string) error {
	...
	api, err := j.dialControllerAsServiceLogged(ctx, &incomingModel.TargetController)
	if err != nil {
...
func (j *JujuManager) AdoptResources(ctx context.Context, user *openfga.User, modelUUID string, sourceControllerVersion version.Number) error {
	...
	api, err := j.dialControllerAsServiceLogged(ctx, &model.Controller)
	if err != nil {
		return fmt.Errorf("failed to dial controller: %w", err)
	}
	defer api.Close()
...
func (j *JujuManager) Activate(ctx context.Context, user *openfga.User, modelTag names.ModelTag, migrationInfo coremigration.SourceControllerInfo, relatedModels []string) error {
	...
	api, err := j.dialControllerAsServiceLogged(ctx, &modelMigration.TargetController)
	if err != nil {
		return fmt.Errorf("failed to dial controller: %w", err)
	}
	defer api.Close()
...
func (j *JujuManager) Import(ctx context.Context, user *openfga.User, serialized jujuparams.SerializedModel) error {
	...
	api, err := j.dialControllerAsServiceLogged(ctx, &incomingMigration.TargetController)
	if err != nil {
		return fmt.Errorf("failed to dial controller: %w", err)
	}
	defer api.Close()
